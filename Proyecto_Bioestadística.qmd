...
-   "VICTIMA_GESTANTE": Muestra una correlación positiva moderada (0.39) con la violencia sexual. Esto sugiere que el estado de gestación de la víctima podría ser un predictor importante.

```{=html}
<!-- -->
```
-   "TRABAJA_AGRESOR": Presenta una correlación positiva débil a moderada (0.15), indicando que la situación laboral del agresor podría tener cierta capacidad predictiva.

```{=html}
<!-- -->
```
-   "NIVEL_EDUCATIVO_VICTIMA": Muestra una correlación positiva débil (0.12), sugiriendo que el nivel educativo de la víctima podría ser un factor predictor, aunque menos fuerte que los anteriores.

```{=html}
<!-- -->
```
-   "SEXO_AGRESOR": Tiene una correlación débil positiva (0.06), lo que podría indicar una ligera relevancia predictiva.

```{=html}
<!-- -->
```
-   "NIVEL_EDUCATIVO_AGRESOR": Muestra una correlación débil positiva (0.08), sugiriendo que el nivel educativo del agresor podría tener algún valor predictivo.

```{=html}
<!-- -->
```
-   "AGRESOR_VIVE_CASA_VICTIMA": Presenta una correlación débil positiva (0.23), lo que podría ser un factor predictor a considerar.

### Selección de variables

Utilizaremos solo 6 variables para la regresión:
...
Data_Final <- Dataimport %>%
  select(VIOLENCIA_SEXUAL_BINARIA, VICTIMA_GESTANDO, TRABAJA_AGRESOR, NIVEL_EDUCATIVO_VICTIMA, SEXO_AGRESOR, AGRESOR_VIVE_CASA_VICTIMA) %>%
...
# Modelos de primer orden
modelo_1.1 <- glm(VIOLENCIA_SEXUAL_BINARIA ~ VICTIMA_GESTANDO, data = Data_Final, family = binomial)
summary(modelo_1.1)
exp(cbind(coef(modelo_1.1), confint(modelo_1.1)))

modelo_1.2 <- glm(VIOLENCIA_SEXUAL_BINARIA ~ TRABAJA_AGRESOR, data = Data_Final, family = binomial)
summary(modelo_1.2)
exp(cbind(coef(modelo_1.2), confint(modelo_1.2)))

modelo_1.3 <- glm(VIOLENCIA_SEXUAL_BINARIA ~ NIVEL_EDUCATIVO_VICTIMA, data = Data_Final, family = binomial)
summary(modelo_1.3)
exp(cbind(coef(modelo_1.3), confint(modelo_1.3)))

modelo_1.4 <- glm(VIOLENCIA_SEXUAL_BINARIA ~ SEXO_AGRESOR, data = Data_Final, family = binomial)
summary(modelo_1.4)
exp(cbind(coef(modelo_1.4), confint(modelo_1.4)))

#modelo_1.5 <- glm(VIOLENCIA_SEXUAL_BINARIA ~ NIVEL_EDUCATIVO_AGRESOR, data = Data_Final, family = binomial)
#summary(modelo_1.5)
#exp(cbind(coef(modelo_1.5), confint(modelo_1.5)))

modelo_1.6 <- glm(VIOLENCIA_SEXUAL_BINARIA ~ AGRESOR_VIVE_CASA_VICTIMA, data = Data_Final, family = binomial)
summary(modelo_1.6)
exp(cbind(coef(modelo_1.6), confint(modelo_1.6)))

# Comparación de AIC
models1 <- list(modelo_1.1, modelo_1.2, modelo_1.3, modelo_1.4, modelo_1.6)
mod.names1 <- c('st.gestante', 'st.trabaja_agresor', 'st.edu_victima', 'st.sexo_agresor', 'st.edu_agresor', 'st.vive_con')
...
```{r}
fm <- glm(VIOLENCIA_SEXUAL_BINARIA~., data = Data_Final, family = binomial, na.action = "na.fail")
